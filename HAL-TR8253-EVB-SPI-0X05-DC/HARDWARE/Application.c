#define _Application_ 1
#include <string.h>
#include "ecat_pdo_config.h"
#include "Application.h"
#undef _Application_
#include "SSC-Device.h"
#include "TR8253Evb.h"

/*******************************************************************************
* @Function Name	: DO_LED_Ctrl
* @Description  	: Main application function, called periodically
* @param        	: void
* @retval       	: void
* @date         	: 2020/6/22
*******************************************************************************/
void DO_LED_Ctrl()
{
	UINT16 v = DOUnit20x7000.aEntries[0];
	DOCH0=(v>>0)&1; DOCH1=(v>>1)&1; DOCH2=(v>>2)&1; DOCH3=(v>>3)&1;
	DOCH4=(v>>4)&1; DOCH5=(v>>5)&1; DOCH6=(v>>6)&1; DOCH7=(v>>7)&1;
	DOCH8=(v>>8)&1; DOCH9=(v>>9)&1; DOCH10=(v>>10)&1; DOCH11=(v>>11)&1;
	DOCH12=(v>>12)&1; DOCH13=(v>>13)&1; DOCH14=(v>>14)&1; DOCH15=(v>>15)&1;
	
	// Copy DO units value to DI units
	memcpy(&DIUnit10x6000, &DOUnit20x7000,ECAT_PDO_TOTAL_BYTES);
}

void DO_LED_Off()
{
	DOCH0 = 0;
	DOCH1 = 0;
	DOCH2 = 0;
	DOCH3 = 0;
	DOCH4 = 0;
	DOCH5 = 0;
	DOCH6 = 0;
	DOCH7 = 0;
	
	DOCH8 = 0;
	DOCH9 = 0;
	DOCH10 = 0;
	DOCH11 = 0;
	DOCH12 = 0;
	DOCH13 = 0;
	DOCH14 = 0;
	DOCH15 = 0;
}

//LED initialization
#if 0
//This function has already defined in function MX_GPIO_Init()
void GPIO_HW_Init()
{
	GPIO_InitTypeDef   GPIO_InitStructure;

	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);// Enable GPIOD Clock   
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOE, ENABLE);// Enable GPIOE Clock	
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_8 | GPIO_Pin_9 | 
																GPIO_Pin_10 | GPIO_Pin_14 | GPIO_Pin_15; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100M
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 |
																GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15; 
	GPIO_Init(GPIOE, &GPIO_InitStructure);	
}
#endif

//// The slave PDO data received by the master station is mapped to the COE
void APPL_CoeTxPdoMapping(uint16_t* pData)
{
	UINT16 *pInputs = (UINT16 *)pData;
	*pInputs++ = *(&DIUnit10x6000.u16SubIndex0 + 1);
}

// The primary station sends PDO data to the secondary station and maps it to the COE
void APPL_CoeRxPdoMapping(uint16_t* pData)
{
	memcpy(&DOUnit20x7000.u16SubIndex0 + 1, pData, ECAT_PDO_TOTAL_BYTES);
}
